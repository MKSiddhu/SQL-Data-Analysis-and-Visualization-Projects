...
-- We need to figure out on which day we need schedule an ad campgain
SELECT 
...
SELECT date_format(created_at,'%W') AS 'Days of the week', COUNT(*) AS 'No. of Registration'
FROM users
GROUP BY 1
ORDER BY 2 DESC
LIMIT 2;
...
-- we will find the users who have never posted a photo.
SELECT username 
FROM users
